...
Notice that the hadoop filesystem and the maven are installed inside the container, so you need to enter the container by 
```
sudo docker start cloudera_quickstart
sudo docker exec -w /host -it cloudera_quickstart bash
```
 and then run the compilation and hadoop commands.
